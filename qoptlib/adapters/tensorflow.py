"""TensorFlow adapter for QOptLib optimizers.

Provides a bridge between TensorFlow/Keras models and QOptLib optimizers.
...
class TensorFlowAdapter:
    """Adapter for using QOptLib optimizers with TensorFlow/Keras models.

    Converts TensorFlow model parameters to/from NumPy arrays,
    ...
    def get_bounds(
        ...
        bounds = []
        for var in self.model.trainable_variables:
            n_elements = int(np.prod(var.shape))
    ...
    def optimize(
        ...
        loss_fn : callable
            Loss function that takes (targets, outputs).
        dataset : tf.data.Dataset or iterable
            Training data.
        iterations : int
        ...
        best_loss : float
            Best loss achieved.
        """
        param_shapes = [tuple(var.shape) for var in self.model.trainable_variables]
